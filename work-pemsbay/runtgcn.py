...
def train_epoch(training_input, training_target, batch_size):
    ...
    torch.save(net.state_dict(), '/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/temp/'+'tempmodel.pth')
...
outprint = '/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/out/%s'%(model_name)
...
if __name__ == '__main__':
    ...
    for epoch in range(training_epoch):
        ...
        with torch.no_grad():
            ...
            net2 = TGCN2(A_wave.shape[0], 1,
                gru_units, seq_len, pre_len).to(device=device_cpu)  #定义网络
            net2.load_state_dict(torch.load('/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/temp/'+'tempmodel.pth', map_location=lambda storage, loc: storage))
            net2.to(device_cpu)
            net2.eval()


            val_input = testX.to(device=device_cpu)
            val_input = val_input.permute(1, 0, 2)
            val_target = testY.to(device=device_cpu)
            h0 = torch.zeros(val_input.size(1), num_nodes, gru_units).to(device=device_cpu)
            A_wave2 = A_wave.to(device=device_cpu,dtype=torch.float32)
            pred = net2(A_wave2, val_input, h0)
            ...
            print("pred shape is:")
            print(pred.shape)
            print("val_target shape is:")
            print(val_target.shape)
#             pred_cpu = pred.reshape(-1,num_nodes)
#             target_cpu = val_target.reshape(-1,num_nodes)
            
  
            
            
            pred_cpu = pred.detach().numpy().reshape(-1,num_nodes)
            target_cpu = val_target.detach().numpy().reshape(-1,num_nodes)
            print("pred_cpu shape is:")
            print(pred_cpu.shape)
            print("target_cpu shape is:")
        ...
        if (epoch % 1000 == 0):
            torch.save(net,'/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/torchimage/model_epoch'+str(epoch)+'.pkl')


    index = validation_rmse.index(np.min(validation_rmse))  # 找出testrmse中最小的那个epoch
    test_result = validation_pred[index]
    var = pd.DataFrame(test_result)
    var.to_csv('/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/torchimage/'+'/test_result_epoch'+str(index)+'.csv',index = False,header = False)
    print(validation_label.shape)
    print(test_result.shape)
    plot_result_3ave(test_result, validation_label, '/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/torchimage/')  # shape为[testbatch*prelen, num_nodes]
    plot_error(training_rmses, training_losses, validation_rmse, validation_acc, validation_maes, '/home/cseadmin/yindu/github/save/tgcn-pytorch-pemsbay/torchimage/')